 ...
 *    保留网段/云元数据地址拦截。URL 主机为 IP 字面量时精确判网段；为域名时至少拦掉
 *    `localhost`。
 * 2. {@link fetchRemoteMediaToBuffer} —— 先做上面的 URL 校验，再带超时下载，并在
 ...
 *   「URL 里直接写内网/元数据 IP 或 localhost」这一最常见、最省事的攻击面。
 ...
 * - 主机是域名时，拦截 `localhost`（及 `*.localhost`）。
 *
...
export function assertSafeRemoteUrl(rawUrl: string): URL {
  ...
  // WHATWG URL 会把 IPv6 主机保留方括号，这里剥掉再判断；同时统一小写。
  ...
  if (host === 'localhost' || host.endsWith('.localhost')) {
    throw new Error(`拒绝下载指向本机（localhost）的媒体 URL：${rawUrl}`)
  }
...
export async function fetchRemoteMediaToBuffer(
  ...
  if (!Number.isFinite(maxBytes) || maxBytes <= 0) {
    throw new Error('fetchRemoteMediaToBuffer: maxBytes 必须为正数')
  }
  ...
  let res: Response
  try {
    res = await fetchFn(url.href, { signal: AbortSignal.timeout(timeoutMs), redirect: 'error' })
  } catch (err) {
    if (isAbortError(err)) {
  ...
  const buffer = await readCapped(res, maxBytes, rawUrl, timeoutMs)
...
async function readCapped(
  ...
  const body = res.body
  if (!body) {
    // 极少数运行时可能没有可读流：退回整包读取但仍强制上限。
    const buf = Buffer.from(await res.arrayBuffer())
    if (buf.length > maxBytes) {
      throw new Error(`远程媒体大小 ${toMB(buf.length)}MB 超过上限 ${toMB(maxBytes)}MB，已丢弃：${rawUrl}`)
    }
    return buf
  }
...
function isBlockedIpv4(octets: [number, number, number, number]): boolean {
  ...
  if (a === 192 && b === 0 && octets[2] === 0) return true // 192.0.0.0/24 IETF 协议保留
