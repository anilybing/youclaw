...
| 向量库 | 定位 | 适用场景 |
|--------|------|---------|
| **Milvus** | 分布式、大规模、生产级 | 十亿级向量、高并发检索 |
| **PGVector** | PostgreSQL 扩展 | 已有 PG 栈、中小规模、事务+向量一体 |
| **Qdrant** | Rust 实现、过滤强 | 带 payload 过滤的语义检索、云原生 |
| **Chroma** | 轻量、嵌入式、开发友好 | 原型/POC、嵌入式 RAG |
| （国际版另见 Weaviate / Pinecone / Redis Stack） | | |

**模型分层路由**：按任务复杂度与成本动态选模——简单分类/抽取走小模型（时延低、成本≈0），复杂推理/规划走大模型或推理模型；以路由策略平衡效果、时延与 token 成本。

**AI 评测与护栏**：
- **Prompt 注入 / 越狱防护**：输入净化、权限最小化、敏感操作二次确认
- **红队 (Red Teaming)**：对抗样本、越狱尝试、数据泄露测试，纳入上线前评估
- **评测体系**：基于 RAGAS / 客观指标（faithfulness / answer relevancy）+ 业务指标 + 人工抽检

**AI 成本治理**：
- 按 token / 推理调用核算成本，建立"每问答成本 / 每任务成本"看板
- 缓存命中、小模型分流、上下文压缩、批处理降低单位成本
- 预算告警与异常检测（参考 FinOps 三阶段，延伸至 AI 支出）

**LLMOps / AI 网关**：模型版本管理、灰度发布、流量路由、限流熔断、审计日志、评测回归、Prompt 版本化——统一在 AI 网关收敛。

> **配套模板**：新增 `templates/ai-adr-template.md`（AI 选型 ADR 模板），覆盖"是否引入 Agent / 选哪家模型 / 向量库与 RAG 策略 / 护栏与成本红线"等高风险决策的标准化记录。
...
| 指标 | 数据 | 来源 |
|------|------|------|
| 云成本管理为首要挑战的企业占比 | **84%** | Flexera 2025/2026 |
| 云成本超出预算的企业占比 | **67%** | Flexera 2025/2026 |
| 至少有10%云支出浪费的企业占比 | **82%** | Flexera 2025/2026 |
| 通过FinOps可实现的平均节省 | **20-30%** | FinOps Foundation |
| 已有FinOps专业团队的企业占比 | **59%**（较2024年51%上升8pp） | Flexera 2025 |
| FinOps实际渗透率（截至2026） | >60%中大型企业已建立FinOps职能 | Flexera 2025/FinOps Foundation |
| 原Gartner预测"2026年70%企业采用FinOps" | 实际渗透率约60%+，达标但低于2022预测 | Flexera 2025/FinOps Foundation
...
## 2026 信创全栈厂商清单（Xinchuang Full-Stack Vendor Reference）

信创（信息技术应用创新）已进入全栈国产化替代深水区。以下为截至2026年 Q2 的国产化主流厂商清单，覆盖芯片→操作系统→数据库→中间件全栈：

### 芯片 (CPU)

| 厂商 | 架构 | 典型型号 | 适用场景 |
|------|------|---------|---------|
| **华为鲲鹏 (Kunpeng)** | ARM v8.x | Kunpeng 920/930 | 服务器、云计算、大数据 |
| **海光 (Hygon)** | x86 兼容 | Hygon C86 3xxx/5xxx | 服务器、高性能计算（x86生态兼容） |
| **飞腾 (Phytium)** | ARM v8 | FT-2000/4, S2500 | 桌面终端、低功耗服务器 |
| **龙芯 (Loongson)** | LoongArch | 3A6000/3C6000 | 桌面终端、嵌入式 |
| **兆芯 (Zhaoxin)** | x86 兼容 | KX-7000/KH-40000 | 桌面终端、办公 |
| **申威 (SW)** | Alpha 自研 | SW26010Pro | 超算、国防/科研专用 |

### 操作系统 (OS)

| 厂商 | 产品 | 技术路线 | 典型部署 |
|------|------|---------|---------|
| **麒麟软件** | 银河麒麟（桌面/服务器） | Linux (openEuler/Ubuntu 衍生) | 党政/央企办公、服务器 |
| **统信软件** | 统信 UOS（桌面/服务器） | Linux (Deepin 衍生) | 党政/金融/教育桌面、服务器 |
| **华为 EulerOS** | openEuler | Linux 开源社区版 | 国产化服务器/云原生 |
| **中科方德** | 方德桌面/服务器 | Linux | 教育/特种行业 |

### 数据库 (Database)

| 厂商 | 产品 | 技术路线 | 适用场景 |
|------|------|---------|---------|
| **达梦 (Dameng)** | DM8/DM9 | 自研关系型 | OLTP 核心业务（政务/金融） |
| **OceanBase** | OceanBase (蚂蚁) | 分布式 HTAP | 金融级核心交易、高并发 OLTP |
| **华为 GaussDB** | GaussDB | 分布式关系型+向量 | 政企/金融/运营商核心系统 |
| **人大金仓** | KingbaseES | PostgreSQL 衍生 | OLTP（政务/军工） |
| **南大通用** | GBase 8a/8t | 分析型+事务型 | 大数据/数据仓库 |
| **TiDB** | TiDB (PingCAP) | 分布式 HTAP (开源) | 互联网/金融弹性扩展 |
| **SequoiaDB** | SequoiaDB (巨杉) | 分布式文档型 | 多模数据/金融非结构化数据 |
| **openGauss** | openGauss (华为开源) | 社区开源关系型 | 技术自主可控首选开源方案 |

### 中间件 (Middleware)

| 厂商 | 产品 | 技术路线 | 适用场景 |
|------|------|---------|---------|
| **东方通** | TongWeb/TongLINK/Q | Java EE 应用服务器/消息中间件 | 政务/金融/电信 |
| **宝兰德** | BES Application Server | Java EE | 电信/政务 |
| **普元** | Primeton Platform | 低代码/集成平台 | 企业数字化转型 |
| **中创中间件** | InforSuite | Java EE 应用服务器 | 政务/国防 |

> **售前方案应用**：央国企/政府客户方案中，技术架构图应标注信创适配层级（"芯片→OS→数据库→中间件"各层国产化替代方案和断点），并注明可替代的国外对标产品（如"达梦→Oracle"、"银河麒麟→Windows Server"、"OceanBase→Oracle RAC"）。

---

## 商用密码应用安全性评估（密评/Crypto Assessment）

### 密评概述

密评（商用密码应用安全性评估）是依据《商用密码管理条例》（2023年7月1日施行）和《商用密码应用安全性评估管理办法》对关键信息基础设施、政务信息系统等开展的密码应用合规性评估，是等保 2.0 之外的另一道合规门槛。

### 密评"三同步"原则

| 原则 | 含义 | 售前方案要求 |
|------|------|------------|
| **同步规划** | 密码应用与信息系统同步规划 | 技术架构中明确密码模块（国密算法 SM2/SM3/SM4/SM9）的部署位置 |
| **同步建设** | 密码应用与信息系统同步建设 | 实施方案中列出密码设备/服务的采购与部署计划 |
| **同步运行** | 密码应用与信息系统同步运行 | 运维计划中涵盖密码设备巡检、证书管理、密钥轮换 |

### 密评四要素

| 要素 | 评定项 | 关键要求 | 典型产品 |
|------|--------|---------|---------|
| **密码算法** | 使用的密码算法是否合规 | 必须使用 SM2/SM3/SM4/SM9 系列国密算法，禁用 RSA-1024/MD5/SHA-1 | 国密浏览器、国密 SSL 网关 |
| **密码协议** | 使用的密码协议是否合规 | TLS 必须使用国密套件（GMTLS/GMSSL），VPN 使用 IPSec 国密模式 | 国密 VPN、签名验签服务器 |
| **密码产品** | 使用的密码产品是否获认证 | 密码产品须获国家密码管理局商用密码产品认证 | 服务器密码机、密钥管理系统(KMS) |
| **密码服务** | 密码管理是否规范 | 密钥全生命周期管理、证书管理、审计日志、应急响应 | 云密码服务平台 |
...
|------|------|---------|
| **第一级** | 自主评估 | 一般信息系统 |
| **第二级** | 委托评估机构评估 | 等保二级及以上系统 |
| **第三级** | 委托评估机构评估（更严格） | 等保三级/关键信息基础设施 |
| **第四级** | 国家密码管理局指定机构评估 | 最高安全等级系统 |

> **售前方案应用**：
> - 政务/央企/金融客户的项目建议书中，应附"密评合规性说明"——明确本方案使用的密码算法（国密SM系列）、密码产品认证情况、密码应用方案框架；
> - 部署架构图中标注国密SSL网关、服务器密码机、密钥管理系统等密码基础设施的位置；
> - 等保+密评"双合规"是政务信息化项目的强制门槛，不可遗漏；
> - 云密码服务（Cryptography as a Service, CaaS）是近年趋势，通过云平台统一提供密码能力，降低各系统重复建设成本。
...
| 版本 | 日期 | 变更说明 |
|------|------|----------|
| V1.2.0 | 2026-07-07 | 例行迭代升级：补AI原生架构、刷新时效数据、修正版本矛盾 |
...
| V1.0.1 | 2026-06-02 | 重大升级：整合 SA Playbook 方法论、C4+4+1+TOGAF 架构体系、SPIN 需求挖掘、ADR 决策记录、完整 30+ 交付物矩阵、13 类专业图表（含 C4 上下文图/容器图、DFD 多层级）、专业 draw.io 制图规范、6 套 PPT 配色方案、RFP 响应、PoC 方案。基于互联网最佳实践和业界标准方法论全面重写。（已于 2026-07-07 更正：原误标为 V2.0，其日期 2026-06-02 早于现版 V1.1.0 的 2026-06-16，属版本号时序矛盾；按现版口径统一为 V1.0→V1.1.0 时序，本条即"重大升级"基线，版本号更正为 V1.0.1 以对齐时序） |
